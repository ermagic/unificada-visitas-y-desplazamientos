...
def mostrar_planificador():
    ...
    with tab_global:
        ...
        cal_date = st.date_input("Selecciona la semana a visualizar:", value=start_of_next_week, format="DD/MM/YYYY", key="cal_date")
        ...
        if df_all.empty:
            st.info("No hay visitas programadas para la semana seleccionada.")
        else:
            with st.container(border=True):
                ...
                def get_assignee(row):
                    if row['status'] == 'Asignada a Supervisor':
                        return f"Supervisor / {row['Coordinador']}"
            ...
            with st.container(border=True):
                st.markdown("##### 🗺️ Visualización en Mapa y Calendario")
    ...
    with tab_planificar:
        st.subheader("Añadir o Editar Mis Visitas Propuestas")
        ...
        with st.container(border=True):
            ...
            st.info("💡 Añade nuevas filas o edita las existentes. Las direcciones se geolocalizarán al guardar.")
            ...
            column_config = {
                ...
                "franja_horaria": st.column_config.SelectboxColumn("Franja Horaria", options=sorted(set(HORAS_LUNES_JUEVES + HORAS_VIERNES)), required=True),
                "direccion_texto": st.column_config.TextColumn("Ubicación (Dirección completa)", required=True, width="large"),
                "equipo": st.column_config.TextColumn("Equipo/Tarea", required=True),
                "observaciones": st.column_config.TextColumn("Observaciones")
            }
            
            edited_df = st.data_editor(
                df, column_config=column_config, num_rows="dynamic", use_container_width=True, key=f"editor_{start}", height=38*(len(df)+4)
            )

            if st.button("💾 Guardar Cambios", type="primary", use_container_width=True):
                with st.spinner("Guardando y geolocalizando direcciones..."):
