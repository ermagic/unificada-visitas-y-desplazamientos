# Fichero: planificador.py (Versión con vista global mejorada para supervisor)
...
def geocode_address(address: str):
    ...
    try:
        geolocator = Nominatim(user_agent="streamlit_app_planner_v14")
...
def mostrar_planificador():
    ...
    # --- Lógica de roles para las pestañas ---
    # Los coordinadores ven sus tareas primero. Los supervisores ven la vista global primero.
    ...
    with tab_mis_visitas:
        ...
        if df_mis.empty:
            ...
            if not propias_asignadas.empty:
                ...
                st.dataframe(propias_asignadas[['fecha_fmt', 'franja_horaria', 'direccion_texto', 'equipo']].rename(columns={
                    'fecha_fmt': 'Fecha', 'franja_horaria': 'Franja', 'direccion_texto': 'Ubicación', 'equipo': 'Equipo'
                }), use_container_width=True, hide_index=True)

            asignadas_supervisor = df_mis[df_mis['status'] == 'Asignada a Supervisor']
            if not asignadas_supervisor.empty:
                st.markdown("####  superviseur ✅ Visitas Asignadas a Martín")
            ...
            if not pendientes.empty: st.markdown("#### ⏳ Pendientes de Asignar"); st.dataframe(pendientes[['fecha_fmt', 'franja_horaria', 'direccion_texto', 'equipo']], use_container_width=True, hide_index=True)
            borradores = df_mis[df_mis['status'] == 'Propuesta']
            if not borradores.empty: st.markdown("#### ✍️ Tus Borradores"); st.dataframe(borradores[['fecha_fmt', 'franja_horaria', 'direccion_texto', 'equipo']], use_container_width=True, hide_index=True)

    with tab_planificar:
        st.subheader("Gestiona tus visitas propuestas (Borradores)")
        # Esta pestaña es principalmente para coordinadores. Supervisor la ve pero no la usa.
        if st.session_state['rol'] != 'coordinador':
        ...
        with col1:
            if st.button("💾 Guardar cambios", type="primary", disabled=(edited.empty or st.session_state['rol'] != 'coordinador')):
        ...
        with col2:
            if st.button("✅ Enviar a supervisor", disabled=(edited.empty or st.session_state['rol'] != 'coordinador')):
    ...
    with tab_global:
        ...
        if df_all.empty:
            ...
            df_all.rename(columns={'nombre_completo': 'Coordinador', 'status': 'Estado', 'equipo': 'Equipo', 'direccion_texto': 'Ubicación'}, inplace=True)
            ...
            st.dataframe(df_filtered[['Coordinador', 'Estado', 'Equipo', 'Ubicación', 'fecha', 'franja_horaria', 'fecha_asignada', 'hora_asignada']], use_container_width=True)

            st.markdown("---")
            col_map, col_cal = st.columns(2)
            with col_map:
                st.subheader("🗺️ Mapa")
                ...
                else: st.info("No hay visitas con coordenadas para mostrar.")
